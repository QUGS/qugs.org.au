...
memb = [
	{bgg:"BradleySigma", disc:"@BradleySigma#7868"},
...
function table()
{
	...
	bgg_gam.onreadystatechange=function()
	{
		...
		if (bgg_gam.readyState==4 && bgg_gam.status==200)
		{
			...
			{
				var g = details[i];
				out += "<tr><td><a href=https://boardgamegeek.com/boardgame/"+g.id+">"+g.name+"</a></td><td>"+g.minp+(g.minp == g.maxp ? "" : "-"+g.maxp)+"</td><td>"+(g.time >= 60 ? ((g.time/60)|0) + " hr " : "")+(g.time%60 == 0 ? "" : g.time%60+ " min")+"</td>"+(g.rate == 0 ? "<td style='color:#808080;'>N/A</td>" : "<td>"+g.rate.toFixed(2)+"</td>")+"<td>"+g.owner+"</td></tr>\n";
			}
			out += "</table>";
